...
void ImageConverter::imageCb(const sensor_msgs::ImageConstPtr& msg){
        ...
        double camera_x = .545; //calculated empirically
        double camera_y = .320; //calculated empirically
        double scale = .00306; // m / px
        double theta = 0.202; // calculated empirically, the angle in rads to turn camera point to world point
        double block_x_camera = scale * (i_centroid - 320);
        double block_y_camera = - scale * (j_centroid - 240);

        block_pose_.pose.position.x = camera_x + (block_x_camera * cos(theta)) + (block_y_camera * sin(theta)); 
        block_pose_.pose.position.y = camera_y - (block_x_camera * sin(theta)) + (block_y_camera * cos(theta)); 
        ...
        for (int i = 0; i < cv_ptr->image.cols; i++) {
            for (int j = 0; j < cv_ptr->image.rows; j++) {
                rgbpix = cv_ptr->image.at<cv::Vec3b>(j, i); //extract an RGB pixel
                blueval = rgbpix[0];
                if (blueval == 255) { //only true for points on the block or centroid
                	//calculate world position of point
                	double point_x = (scale * (i - 320) * cos(theta)) + (scale * (j - 240) * sin(theta));
                	double point_y = (scale * (i - 320) * cos(theta)) + (scale * (j - 240) * sin(theta));
                	covariance_matrix(0,0) += point_x * point_x;
                	covariance_matrix(1,0) += point_x * point_y;
                	covariance_matrix(0,1) += point_x * point_y;
                	covariance_matrix(1,1) += point_y * point_y;
                	pix++;
                } else { //else ignore
                }
            }
        }
        covariance_matrix(0,0) = covariance_matrix(0,0) / pix;
        covariance_matrix(1,0) = covariance_matrix(1,0) / pix;
        covariance_matrix(0,1) = covariance_matrix(0,1) / pix;
        covariance_matrix(1,1) = covariance_matrix(1,1) / pix;
        int x = 5;
