...
double goalBlockX = 0.5;
double goalBlockY = 0.5;

void cbBlockPose(const geometry_msgs::PoseStamped& message_holder) {

    block_pose_ = message_holder;
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber block_pose_subscriber = nh.subscribe("block_pose", 1, cbBlockPose);
    ...
    while (ros::ok()) {
        ...
        ros::spinOnce(); //let the subscriber grab the latest data        
        
        //move to approach pose:
        ROS_INFO("moving to approach pose");
        ROS_INFO("blockX = %f",block_pose_.pose.position.x);
        ROS_INFO("blockY = %f",block_pose_.pose.position.y);
        ...
        //move to goal pose
        tool_pose.pose.position.x = goalBlockX; 
        tool_pose.pose.position.y = goalBlockY; 
